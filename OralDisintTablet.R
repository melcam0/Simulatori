...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    # Factor 1: Binder (%, p/p)
    card(
      sliderInput("x1", "Binder (%, p/p)", 
                  min = 2, max = 5, value = 3.5, step = 0.1)
    ),
    
    # Factor 2:  Disintegrant level (%, p/p)
    card(
      sliderInput("x2", " Disintegrant level (%, p/p)", 
                  min = 2, max = 5, value = 3.5, step = 0.1)
    ),
    
    # Factor 3: Compaction force (MPa)
    card(
      sliderInput("x3", "Compaction force (MPa):", 
                  min = 10, max = 20, value = 15, step = 1)
    ),
    
    # Factor 4: Binder type
    card(
      sliderTextInput(
        inputId = "x4",
...
"
    ...
    card(
      ...
      card_header(
        "Historical Data",
        downloadButton("downloadData", "Save to Excel", 
